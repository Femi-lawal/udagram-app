...
const generateEmail = () => `e2e-${Date.now()}-${Math.random().toString(36).substring(7)}@test.com`;
...
test.describe('Authentication API', () => {
  ...
  test.describe('Token Validation', () => {
    ...
    test.skip('should reject missing authorization header', async ({ request }) => {
      // Note: /api/v1/auth/validate endpoint not implemented
      const response = await request.get(`${AUTH_URL}/api/v1/auth/validate`);
      expect(response.status()).toBe(401);
    });
  ...
  test.describe('Gateway Auth Routes', () => {
    ...
    test('should proxy auth registration through gateway', async ({ request }) => {
      const email = generateEmail();
