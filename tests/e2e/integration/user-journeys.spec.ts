...
const generateEmail = () => `e2e-int-${Date.now()}-${Math.random().toString(36).substring(7)}@test.com`;
...
test.describe('End-to-End User Journeys', () => {
  ...
  test.describe.serial('Complete User Registration and Feed Journey', () => {
    ...
    test('Step 1: User registers successfully', async ({ request }) => {
      const response = await request.post(`${GATEWAY_URL}/api/v1/auth/register`, {
        data: { email, password },
      });
    ...
    test('Step 5: User views their created post', async ({ request }) => {
      ...
      const response = await request.get(`${FEED_URL}/api/v1/feed/${createdFeedId}`);
    ...
    test('Step 6: User likes their post', async ({ request }) => {
      ...
      const response = await request.post(`${FEED_URL}/api/v1/feed/${createdFeedId}/like`);
    ...
    test('Step 7: User updates their post', async ({ request }) => {
      ...
      const response = await request.put(`${FEED_URL}/api/v1/feed/${createdFeedId}`, {
        data: {
    ...
    test('Step 8: User can refresh their token', async ({ request }) => {
      ...
      const loginResponse = await request.post(`${GATEWAY_URL}/api/v1/auth/login`, {
        data: { email, password },
      });
      ...
      const response = await request.post(`${GATEWAY_URL}/api/v1/auth/refresh`, {
  ...
  test.describe('Multi-User Interaction', () => {
    ...
    test('Two users can interact with the same feed item', async ({ request }) => {
      // User 1 registers and creates a post
      const user1Email = generateEmail();
      const user1Response = await request.post(`${AUTH_URL}/api/v1/auth/register`, {
      ...
      const likeResponse = await request.post(`${FEED_URL}/api/v1/feed/${feedId}/like`);
      expect(likeResponse.status()).toBe(200);

      // Verify the like count increased
      const getResponse = await request.get(`${FEED_URL}/api/v1/feed/${feedId}`);
  ...
  test.describe('Error Handling Journey', () => {
    ...
    test('User receives proper error for invalid registration', async ({ request }) => {
      const response = await request.post(`${GATEWAY_URL}/api/v1/auth/register`, {
        data: {
          email: 'invalid-email',
          password: '123',
    ...
    test('User receives proper error for invalid login', async ({ request }) => {
      const response = await request.post(`${GATEWAY_URL}/api/v1/auth/login`, {
        data: {
    ...
    test('User receives 404 for non-existent feed item', async ({ request }) => {
      const response = await request.get(`${GATEWAY_URL}/api/v1/feed/00000000-0000-0000-0000-000000000000`);
      expect(response.status()).toBe(404);
    });

    test('User receives error for invalid feed data', async ({ request }) => {
      const email = generateEmail();
      const regResponse = await request.post(`${AUTH_URL}/api/v1/auth/register`, {
